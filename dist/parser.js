...
function detectFormat(filePath) {
    ...
    if (basename === 'CLAUDE.md' || normalized.includes('.claude/CLAUDE.md'))
        return 'claude-md';
    if (basename === '.cursorrules' || normalized.includes('.cursor/rules/'))
        return 'cursorrules';
    if (basename === 'AGENTS.md' || normalized.includes('.github/AGENTS.md'))
        return 'agents-md';
    if (basename === 'GEMINI.md')
        return 'gemini-md';
    if (basename === 'copilot-instructions.md' || normalized.includes('.github/copilot-instructions.md'))
 ...
 */
function extractInstructions(text, baseLineOffset) {
    const instructions = [];
 ...
 */
function extractReferences(text, baseLineOffset) {
    const references = [];
    const lines = text.split('\n');
    const seen = new Set();
    for (let i = 0; i < lines.length; i++) {
        ...
        const barePathMatches = lineWithoutUrls.matchAll(/(?:^|\s)((?:\.\/|src\/|lib\/|app\/|test\/|tests\/|config\/|scripts\/)[^\s,;:)]+)/g);
...
function isFilePath(value) {
    ...
    if (value.includes('/') || value.includes('\\')) {
        ...
        if (/^(?:example|your-project|path\/to)\//i.test(value))
            return false;
        // Skip node_modules, dist, build, .git
        if (/^(?:node_modules|dist|build|\.git)\//i.test(value))
            return false;
        return true;
    }
